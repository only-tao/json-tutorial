...
static void test_parse_array() {
    ...
    // value !!! 
    // lept_free(&v);// if string,free string room. 

    lept_init(&v);// type->null
    EXPECT_EQ_INT(LEPT_PARSE_OK,lept_parse(&v,"[ null , false , true , 123 , \"abc\" ]"));
    EXPECT_EQ_INT(LEPT_ARRAY, lept_get_type(&v));// type
    EXPECT_EQ_SIZE_T(5, lept_get_array_size(&v)); // size 
    ...
    lept_free(lept_get_array_element(&v,4));

    lept_free(&v);
    // 需要递归 free all item room ? 
    // [ [ ] , [ 0 ] , [ 0 , 1 ] , [ 0 , 1 , 2 ] ]
    lept_init(&v);

    EXPECT_EQ_INT(LEPT_PARSE_OK,lept_parse(&v,"[ [ ] , [ 0 ] , [ 0 , 1 ] , [ 0 , 1 , 2 ] ]"));
    EXPECT_EQ_INT(LEPT_ARRAY, lept_get_type(&v));// type
    EXPECT_EQ_SIZE_T(4, lept_get_array_size(&v)); // size
    for(int i=0;i<4;i++){
        lept_value* a = lept_get_array_element(&v,i);
        EXPECT_EQ_INT(LEPT_ARRAY, lept_get_type(a));// type
        EXPECT_EQ_SIZE_T(i, lept_get_array_size(a)); // size
        for(int j=0;j<i;j++)
        {
